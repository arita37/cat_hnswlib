...
public:
    ...
    void addItems(py::object input, py::object ids_ = py::none(), int num_threads = -1) {
        ...
        if (!ids_.is_none()) {
            ...
            if(ids_numpy.ndim==1 && ids_numpy.shape[0]==(int)rows) {
                ...
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                        for (size_t ncondition = 0; ncondition < conditions.size(); ncondition++) {
                            ...
                            while(!result.empty()){
                                ...
                                vd.push_back(result_tuple.first);
                                if (mapid.empty())
                                    vl.push_back(std::to_string(result_tuple.second));
                                else
                                    vl.push_back(mapid[result_tuple.second]);
                                result.pop();
                            }
                            std::reverse(vd.begin(),vd.end());
                            std::reverse(vl.begin(),vl.end());
                            // Insert vd and vl to data_numpy_d and data_numpy_l
...
//
            ...
            if (normalize==false) {
                ...
                ParallelFor(0, conditions.size(), num_threads, [&](size_t ncondition, size_t threadId) {
                        ...
                        while(!result.empty()){
                            auto result_tuple = result.top();
                            data_numpy_d.at(ncondition).push_back(result_tuple.first);
                            data_numpy_l.at(ncondition).push_back(result_tuple.second);
                            result.pop();
                        }
                        std::reverse(data_numpy_d.at(ncondition).begin(),data_numpy_d.at(ncondition).end());
                        std::reverse(data_numpy_l.at(ncondition).begin(),data_numpy_l.at(ncondition).end());
    ...
    void indexTagged(size_t tag, size_t m = 0) {
